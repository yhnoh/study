...
외부 API 연동을 완료하고 배포했는데, 로컬에서는 잘 됐던 API 호출이 서버에서만 실패하게 되는 경우 가장 먼저 확인해야 할 것은 네트워크 연결 문제다. 서버에서 외부 API로 나가는 트래픽이 방화벽이나 보안그룹에 의해 차단되었거나, DNS 조회가 실패했거나, 라우팅 문제가 있을 수 있다. <br/>
DNS를 잘못 작성하는 경우에는 코드를 수정해서 재배포하면 되지만, 네트워크 문제는 서버의 네트워크 설정을 직접 확인하고 수정해야 하기 때문에, 네트워크에 대한 기본적인 이해가 없다면 문제의 원인을 찾는 데 큰 어려움을 겪게 된다. <br/>

### 내 네트워크 정보 확인
네트워크 정보를 확인할 때는 가장 먼저 서버의 네트워크 정보를 먼저 확인해야한다. 외부 API로 나가는 트래픽이 어떤 IP로 나가는지, 어떤 포트가 열려있는지, 라우팅 테이블을 확인해서 어디로 나가는지 확인해야 한다. <br/>


#### 내 IP 주소 확인

네트워크 문제를 진단할 때 가장 먼저 확인해야 하는 것이 서버의 IP 주소다. 서버는 일반적으로 Private IP와 Public IP를 가지며, 같은 네트워크 대역의 서버끼리는 Private IP로 통신하고, 외부 API로 나가는 트래픽은 Public IP로 나간다. 어떤 IP로 트래픽이 나가는지 파악해야 방화벽 설정이나 IP 화이트리스트 문제를 정확히 진단할 수 있다. <br/>


먼저 `ifconfig` 명령어로 서버의 네트워크 인터페이스와 IP 주소를 확인한다. `hostname -I` 명령어로 Private IP만 간단하게 확인할 수도 있다. <br/>

```bash
# 서버의 모든 네트워크 인터페이스와 IP 확인
$ ifconfig
eth0: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 9001
      inet 10.0.1.5  netmask 255.255.255.0  broadcast 10.0.1.255
      inet6 fe80::83c:b5ff:fe9a:5e76  prefixlen 64  scopeid 0x20<link>
      ether 0a:3c:b5:9a:5e:76  txqueuelen 1000  (Ethernet)
...
| 항목 | 값 | 의미 |
|---|---|---|
| `eth0` | 인터페이스 이름 | 실제 네트워크 카드. EC2에서는 `eth0`, `ens5` 등으로 표시 |
| `inet` | `10.0.1.5` | 서버의 Private IP 주소 |
| `netmask` | `255.255.255.0` | 같은 네트워크 대역 범위 (`/24`와 동일) |
| `broadcast` | `10.0.1.255` | 같은 네트워크 전체에 브로드캐스트할 때 사용하는 주소 |
| `inet6` | `fe80::...` | IPv6 주소 |
| `ether` | `0a:3c:b5:...` | MAC 주소 (물리적 네트워크 카드 고유 주소) |
| `mtu` | `9001` | 한 번에 전송 가능한 최대 패킷 크기 (바이트) |
| `lo` | 루프백 인터페이스 | 자기 자신을 가리키는 가상 인터페이스. `127.0.0.1` |

현재 서버의 Private IP는 `10.0.1.5`이며 netmask가 `255.255.255.0`이기 때문에 같은 네트워크 대역(`10.0.1.0/24`)을 가진 서버들은 Private IP로 서로 통신이 가능하다. <br/>
또한 자기 자신을 가리키는 루프백 인터페이스(`lo`)가 `127.0.0.1`이기 때문에 같은 서버 내에서 통신할 때에는 `127.0.0.1`로 접근할 수 있다는 것도 알 수 있다. <br/>
내부 네트워크 대역과 Private IP를 확인했으니, 다음은 외부로 나가는 Public IP를 확인해야 한다.
    
<br/>

```bash

# 외부에서 보이는 Public IP 확인
$ curl -s ifconfig.me

# 외부에서 보이는 Public IP 와 위치 정보 확인
$ curl ipinfo.io
```

외부에서 보이는 Public IP를 확인할 수 있다. 이 Public IP는 NAT Gateway의 IP일 수도 있고, 로드밸런서의 IP일 수도 있다. **_외부 API에서 IP 화이트리스트를 설정할 때는 이 Public IP를 등록_**해야 한다.

#### 현재 열려있는 포트 확인

포트는 하나의 서버에서 여러 프로세스가 동시에 통신할 수 있도록 하는 논리적인 통로다. 프로세스는 특정 포트를 열어서 외부와 통신하며 주로 사용하는 웹 서버는 보통 80(HTTP) 또는 443(HTTPS) 포트를 사용한다. 애플리케이션이 정상적으로 포트를 열고 있는지, 의도한 IP 범위에서만 접근 가능한지 확인하기 위해 현재 열려있는 포트를 확인해야 한다. <br/>
`ss` 명령어나 `netstat` 명령어로 현재 열려있는 포트와 해당 포트를 열고 있는 프로세스를 확인할 수 있다. <br/>

```bash
$ ss -tunpl
Netid  State      Recv-Q  Send-Q  Local Address:Port  Peer Address:Port  Process
tcp    LISTEN     0       128     0.0.0.0:22          0.0.0.0:* users:(("sshd",pid=1025,fd=3))
tcp    LISTEN     0       100     127.0.0.1:5432      0.0.0.0:* users:(("postgres",pid=1142,fd=7))
tcp    LISTEN     0       1024    *:8080              *:* users:(("java",pid=3521,fd=96))
udp    UNCONN     0       0       0.0.0.0:68          0.0.0.0:* users:(("dhclient",pid=941,fd=6))
#                         ↑ 127.0.0.1은 로컬에서만 접근 가능
```

위 명령어를 통해서 **_포트가 열려 있는지 확인이 가능하며, 모든 IP에서 접근 가능한지 아니면 특정 IP에서만 접근 가능한지도 확인_**할 수 있다. <br/>
또한 이전에 실행 중이던 프로세스가 포트를 선점하고 있어 애플리케이션이 실행되지 않을 때도 어떤 프로세스가 해당 포트를 점유하고 있는지 확인할 수 있다.


#### 라우팅 테이블

라우팅 테이블은 네트워크 패킷이 목적지까지 어떻게 이동해야 하는지를 정의하는 표다. 같은 서브넷이면 직접 통신하고, 외부로 나가는 패킷은 게이트웨이를 통해 전달된다. 라우팅 테이블이 잘못 설정되어 있거나 게이트웨이가 없다면 외부 API로 나가는 트래픽이 실패할 수 있어 확인이 필요하다. <br/>
`ip route` 명령어로 현재 라우팅 테이블을 확인할 수 있다. <br/>

```bash
$ ip route
default via 10.0.1.1 dev eth0    ← 기본 게이트웨이 (외부로 나갈 때 여기로)
10.0.1.0/24 dev eth0             ← 같은 서브넷은 직접 통신
```

`default via` 뒤의 주소가 게이트웨이다. 이 게이트웨이가 없거나 잘못되어 있으면 외부 통신이 전혀 안 된다.

### 외부 연결 확인

내 서버 상태를 확인했다면, 다음은 **외부까지 연결이 되는지** 단계별로 확인해야한다. 네트워크 문제는 어디서 막혔는지 구간을 좁혀가면서 확인하는 것이 중요하다. <br/>

#### 1. 네트워크 연결 확인

가장 먼저 외부와 네트워크 연결이 되는지 확인해야 한다. `ping` 명령어로 패킷을 보내서 응답이 온다면 네트워크 연결 자체는 문제가 없다고 볼 수 있다. <br/>

```bash
# 구글 DNS에 ICMP 패킷을 보내 인터넷 연결 확인
$ ping -c 4 8.8.8.8
PING 8.8.8.8: 56 data bytes
64 bytes from 8.8.8.8: icmp_seq=0 ttl=117 time=1.5 ms
# 응답이 없으면 → 네트워크 연결 자체가 안 되는 것
```

참고로 `ping` 명령어를 통해서 응답 실패가 오는 경우도 존재하는데, 이때 내 서버의 방화벽이나 보안그룹에서 ICMP 패킷을 차단하고 있을 있고, 외부 서버에서 ICMP 패킷을 차단하고 있을 수도 있다. 따라서 `ping`이 실패한다고 해서 무조건 네트워크 연결이 안 된다고 단정할 수는 없다. <br/>

#### 2. DNS 동작 확인

DNS(Domain Name System)는 도메인 이름을 IP 주소로 변환하는 시스템이다. DNS 조회가 실패하면 도메인 이름으로 외부 API에 접근할 수 없게 된다. `nslookup` 명령어로 도메인 이름이 올바르게 IP로 변환되는지 확인할 수 있다. <br/>
...
sequenceDiagram
    ...
    participant D as DNS 서버 (164.124.101.2)
    participant E as 외부 서버 (google.com)

    S->>D: google.com의 IP가 뭐야? (DNS 조회)
    D-->>S: 142.250.198.174 (IP 응답)
    S->>E: 142.250.198.174로 직접 요청
    E-->>S: 응답
```

`nslookup` 명령어로 DNS 조회가 실패한다면 몇가지 원인이 있을 수 있다.
- `내 서버의 DNS 설정 확인`: 서버가 도메인 주소를 통해서 외부 서버에 접근하게 되면 DNS 서버를 통해서 도메인 이름이 IP 주소로 변환되어야 한다. 이때 내 서버가 올바른 DNS 서버를 사용하고 있는지 확인해야 한다. `/etc/resolv.conf` 파일을 확인해서 올바른 DNS 서버가 설정되어 있는지 확인한다. 
- `외부 API 도메인 확인`: 외부 API의 도메인이 존재하는지, DNS 레코드가 올바르게 설정되어 있는지 확인한다. 도메인이 존재하지 않거나 DNS 레코드가 잘못 설정되어 있다면 DNS 조회가 실패할 수 있다.    


#### 3. 특정 포트가 열려있는지 확인

보통 외부 API는 특정 포트(예: 80, 443)를 통해서 통신하기 때문에, 해당 포트가 열려있는지 확인해야 한다. `nc` 명령어로 특정 호스트의 특정 포트가 열려있는지 확인할 수 있다. <br/>
...
만약 `nc` 명령어로 포트가 닫혀있다고 나온다면, 외부 서버에서 해당 포트를 차단하고 있을 수도 있고, 내 서버에서 나가는 트래픽이 방화벽이나 보안그룹에 의해 차단되고 있을 수도 있다. <br/>


#### 4. HTTP 연결 확인

아마 1번 부터 3번까지를 확인하였다면 이제 네트워크 연결은 문제가 없을 것이다. 이후 HTTP 연결을 통해서 권한이나 인증 문제로 연결이 안 되는지 확인해야 한다. `curl` 명령어로 HTTP 요청을 보내서 응답이 오는지 확인할 수 있다. <br/>

```bash
# HTTP 응답 확인 (-v 옵션으로 연결 과정 상세 출력)
...
### 앞으로 알아야할 개념
방화벽과 보안그룹, NAT, 로드밸런서, 프록시, VPN, 게이트웨이 등 네트워크 관련 개념들은 앞으로 서버 운영에서 자주 등장하는 개념들이기 때문에, 앞으로 하나씩 자세히 알아볼 것이다. <br/>
