...
### 다른 계정으로 명령어 실행하기
현재 내가 가진 계정의 권한으로 명령어 수행이나 파일을 읽거나 쓰는 것이 안 될 때, `sudo -u <계정> <명령어>`를 사용하여 다른 계정으로 명령어를 실행할 수 있다. <br/>

예를 들어 `sudo -u appuser cat /opt/myapp/app.jar`는 `appuser` 계정으로 `app.jar` 파일을 읽는다.
...
| 명령어 | 설명 |
|---|---|
| `whoami` | 현재 내가 누구인지 확인 |
| `id` | UID, GID, 소속 그룹 확인 |
| `ls -al` | 파일 권한과 소유자 상세 확인 |
| `chmod 755 <파일>` | 파일 권한 변경 |
| `chown appuser:appgroup <파일>` | 파일 소유자와 그룹 변경 |
| `sudo -u appuser <명령어>` | 특정 계정으로 명령 실행 |

### 왜 root로 앱을 실행하면 안 되는가

서비스 운영 시 root 계정으로 애플리케이션을 실행하지 않는다. 이유는 단순하다: **root로 실행된 앱이 해킹되면 서버 전체가 위험**하기 때문이다. 해커가 앱의 취약점을 통해 코드를 실행하면, 그 코드는 root 권한으로 동작한다. 서버의 모든 파일을 읽고, 지우고, 백도어를 심을 수 있다.

별도의 서비스 계정을 만들고, 앱에 필요한 디렉토리에만 권한을 부여하는 것이 올바른 방식이다.

```bash
# 서비스 계정 생성 (로그인 불가 shell, 홈 디렉토리 없음)
useradd -r -s /sbin/nologin appuser

# 앱 디렉토리의 소유권을 서비스 계정으로 변경
chown -R appuser:appuser /opt/myapp

# 해당 계정으로 앱 실행
sudo -u appuser java -jar /opt/myapp/app.jar
```

### 장애 시나리오: 배포 후 앱 실행은 됐는데 파일을 읽거나 쓰지 못한다

배포 스크립트를 root로 실행했고, 앱은 `appuser`로 실행 중이다. 앱 로그에 이런 에러가 찍힌다:

```
java.io.FileNotFoundException: /opt/myapp/logs/app.log (Permission denied)
```

**Step 1: 앱이 어떤 계정으로 실행 중인지 확인**
...
**핵심**: 배포 스크립트가 root로 새 디렉토리를 만들면 소유자가 root가 된다. 앱을 실행하기 전에 디렉토리 소유권을 꼭 확인해야 한다.

---

## 2. 프로세스

<!-- > 장애가 발생했을 때 가장 먼저 확인해야 할 것은 프로세스가 살아있는지다. -->

애플리케이션은 서버에서 하나의 **프로세스**로 동작한다. OS는 각 프로세스에 고유한 PID(Process ID)를 부여하고 CPU,메모리를 할당해 관리한다.

프로세스 상태를 이해하면 "앱이 응답 안 함"의 원인을 빠르게 좁힐 수 있다.

### 프로세스 상태

| 상태 | 기호 | 설명 |
|---|---|---|
| Running | R | CPU를 점유하며 실행 중 |
| Sleeping | S | I/O 대기 등으로 일시 중단 (정상) |
| Zombie | Z | 종료됐지만 부모 프로세스가 정리하지 않은 상태 |
| Stopped | T | 시그널에 의해 정지된 상태 |

Sleeping 상태는 정상이다. 대부분의 시간 동안 앱은 I/O(DB 쿼리, HTTP 응답 대기 등)를 기다리며 Sleeping 상태에 있다.

Zombie 상태가 문제다. 프로세스 자체는 종료됐지만 부모 프로세스가 종료 신호를 처리하지 않아 PID를 계속 점유하고 있다. 좀비 프로세스가 쌓이면 PID 고갈로 새 프로세스를 만들 수 없게 된다.
