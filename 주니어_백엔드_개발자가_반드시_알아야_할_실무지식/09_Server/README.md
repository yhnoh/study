...
장애가 발생했을 때 가장 먼저 확인해야 할 것은 프로세스가 살아있는지다. 애플리케이션 로그에 아무것도 남지 않은 채 서비스가 죽었다면, 코드 문제가 아니라 OS 수준에서 프로세스가 강제 종료된 것일 수 있다. <br/>

> 프로세스 상태를 모르면 장애 대응 자체가 어려워진다. 애플리케이션이 응답하지 않는다고 해서 무작정 애플리케이션을 재시작하는 것은 현재 프로세스가 좀비 상태인지, OOM Killer에 의해 죽은 건지, 아니면 단순히 요청이 몰려서 응답이 느린 건지 모르는 상태에서 하는 조치다. <br/>
